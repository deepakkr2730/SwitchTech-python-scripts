...
# Compatibility shim for MoviePy v2: older code expects methods like `set_start`,
# `set_duration`, `set_position`, `set_opacity` which were renamed to `with_start`,
# `with_duration`, `with_position`, `with_opacity` in v2. Patch lightweight adapters
# onto the clip classes so legacy scripts work without rewriting all calls.
try:
    from moviepy.video.VideoClip import VideoClip, ImageClip

    def _set_start(self, t):
        return self.with_start(t)

    def _set_duration(self, d):
        return self.with_duration(d)

    def _set_position(self, p):
        return self.with_position(p)

    def _set_opacity(self, o):
        return self.with_opacity(o)

    def _fx(self, func, *a, **k):
        # Compatibility: allow `clip.fx(vfx.somefx, args...)` where func expects (clip, ...)
        return func(self, *a, **k)

    # Attach if missing
    for cls in (VideoClip, ImageClip):
        if not hasattr(cls, 'set_start'):
            setattr(cls, 'set_start', _set_start)
        if not hasattr(cls, 'set_duration'):
            setattr(cls, 'set_duration', _set_duration)
        if not hasattr(cls, 'set_position'):
            setattr(cls, 'set_position', _set_position)
        if not hasattr(cls, 'set_opacity'):
            setattr(cls, 'set_opacity', _set_opacity)
        if not hasattr(cls, 'fx'):
            setattr(cls, 'fx', _fx)
except Exception:
    # If the new API or imports aren't available, continue — script will fail later with clear error.
    pass

# Compatibility helpers for fx functions (v2 uses PascalCase names like FadeIn)
try:
    import moviepy.video.fx as vfx_mod
    # Provide lowercase function aliases (e.g. fadein -> FadeIn) for backwards compatibility.
    try:
        for name in getattr(vfx_mod, '__all__', []):
            if not name: continue
            # Only map PascalCase names to lowercase aliases
            if name[0].isupper():
                alias = name.lower()
                if hasattr(vfx_mod, name) and not hasattr(vfx_mod, alias):
                    cls_or_func = getattr(vfx_mod, name)
                    # create a wrapper that calls the Effect/class and applies it to clip
                    def make_alias(obj):
                        def alias_func(clip, *a, **k):
                            try:
                                inst = obj(*a, **k)
                                # If instance has apply, use it
                                if hasattr(inst, 'apply'):
                                    return inst.apply(clip)
                                # else if obj itself is a function, call it with clip
                                return obj(clip, *a, **k)
                            except TypeError:
                                # fallback: try calling obj as function with clip first (legacy)
                                return obj(clip, *a, **k)
                        return alias_func
                    setattr(vfx_mod, alias, make_alias(cls_or_func))
    except Exception:
        pass
except Exception:
    pass
...
PY_LOGO_PATH   = r"C:\Users\featb\Downloads\Lenevo to ACER\SwitchTech\mandatory files\python_logo.png"
WATERMARK_PATH = r"C:\Users\featb\Downloads\Lenevo to ACER\SwitchTech\mandatory files\SwitchTech lite.png"
# "C:\Users\featb\Downloads\Lenevo to ACER\SwitchTech\mandatory files\python_logo.png"
