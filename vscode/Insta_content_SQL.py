...
# Compatibility shim for MoviePy v2: provide legacy v1-style methods and fx names
try:
...
# Provide lowercase fx aliases (fadein/fadeout/freeze) mapping to v2 Effect API
try:
    import moviepy.video.fx as vfx_mod
    if not hasattr(vfx_mod, 'fadein') and hasattr(vfx_mod, 'FadeIn'):
        def _fadein(clip, duration, initial_color=None):
            return vfx_mod.FadeIn(duration, initial_color).apply(clip)
        setattr(vfx_mod, 'fadein', _fadein)
    if not hasattr(vfx_mod, 'fadeout') and hasattr(vfx_mod, 'FadeOut'):
        def _fadeout(clip, duration, final_color=None):
            return vfx_mod.FadeOut(duration, final_color).apply(clip)
        setattr(vfx_mod, 'fadeout', _fadeout)
    if not hasattr(vfx_mod, 'freeze') and hasattr(vfx_mod, 'Freeze'):
        def _freeze(clip, t, freeze_duration=0):
            return vfx_mod.Freeze(t, freeze_duration).apply(clip)
        setattr(vfx_mod, 'freeze', _freeze)
except Exception:
    pass
...
ITEM_TEXTS = [
  """Update product prices by latest FX rate >> UPDATE…FROM join || 
UPDATE products p 
SET price_usd = p.price_inr / r.fx_rate 
FROM fx_rates r 
WHERE r.currency = 'INR' 
AND r.as_of = (SELECT MAX(as_of) 
FROM fx_rates WHERE currency = 'INR') 
AND p.currency = 'INR';""",
""""
Detect overlapping bookings >> Self-join on time-ranges || 
...
""",
"""
Find average salary per department >> GROUP BY + AVG || 
...
def main():
    ...
    for k,(title,desc,code) in enumerate(items,1):
        ...
        out_video = os.path.join(args.outdir,f"sql_concept_animation{vid_idx:03d}.mp4")
